...
```
2026-07-12 14:03:11 [praeco] OK message sent
2026-07-12 14:05:44 [praeco] ERROR Telegram API returned HTTP 401
...
Message *contents* are never logged, only metadata (timestamp, status, recipient/subject for praecomail). This keeps the log useful for auditing delivery and troubleshooting without unnecessarily duplicating potentially sensitive alert text.

## Permissions

`/var/log/praeco` is created with mode 2750 (setgid) and owned by root:praeco. This means every log file written into it automatically inherits the `praeco` group, regardless of which user or cron job invokes the commands. To let an operator tail the logs without `sudo`, add their user account to the `praeco` group:
